	       ...
	       FLOAT *result)
{
  ...
  dev_table = clCreateBuffer(cli->context, CL_MEM_READ_ONLY, TABLE_SIZE * 6 * sizeof(FLOAT), NULL, NULL);
  ...
  clSetKernelArg(kernel_hh_calc, 1, sizeof(unsigned long), &i);
  clSetKernelArg(kernel_hh_calc, 2, sizeof(cl_mem), &dev_n);
  clSetKernelArg(kernel_hh_calc, 3, sizeof(cl_mem), &dev_m);
  clSetKernelArg(kernel_hh_calc, 4, sizeof(cl_mem), &dev_h);
  clSetKernelArg(kernel_hh_calc, 5, sizeof(cl_mem), &dev_table);
  clSetKernelArg(kernel_hh_calc, 6, sizeof(cl_mem), &dev_v);
...
int main(int argc, char **argv)
{
  ...
  diff = diffArray(MAX_T/DT, result_host, result_opencl);
  ...
  */
  ...
  hh_save_result(MAX_T/DT, "result_host.txt", result_host);
  hh_save_result(MAX_T/DT, "result_opencl.txt", result_opencl);
